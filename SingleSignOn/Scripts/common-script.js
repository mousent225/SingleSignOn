...
function confirmDeleteAll() {
    return confirm('Do you really want to delete all rows?');
}
/*======================Show notification========================*/
...
var rowIndex = function (row, column, value, defaultHtml, columnSettings, rowData) {
    return '<div style="text-align: center; margin-top: 5px; color: #606060;">' + (row + 1).toString() + '</div>';
}
...
var colHeaderLeft = function (value) {
    return '<div style="text-align: left; margin-top: 5px; margin-left: 5px; font-weight:bold; color: #606060">' + value + '</div>';
}

var colHeaderCenter = function (value) {
    return '<div style="text-align: center; margin-top: 5px; font-weight:bold; color: #606060">' + value + '</div>';
}

var colHeaderRight = function (value) {
    return '<div style="text-align: right; margin-top: 5px; margin-right: 5px;font-weight:bold; color: #606060">' + value + '</div>';
}
...
var rowStatus = function (row, column, value, defaultHtml, columnSettings, rowData) {
    return '<div style="text-align: center; margin-top: 5px; color: #606060;">' + (value ? "Enable" : "Disable") + '</div>';// value ? "Enable" : "Disable";
}
//end -------format collumn
...
//yyyy.MM.dd
function Dt2StrFormat(dt, format) {
    return dt.getFullYear() + format + convert2Degit((dt.getMonth() + 1).toString())  + format + convert2Degit(dt.getDate().toString());
}
...
function fnSetDataDropdown(url, combox, hidenField, cateId, width) {
    var source =
        {
            datatype: "json",
            datafields: [
    ...
    combox.on('select', function (event) {
        var args = event.args;
        if (args != undefined) {
            var item = event.args.item;
            if (item != null) {
                hidenField.val(item.value);
            }
        }
    });
}
function fnSetValueForComboBox(combo, value) {
    var dataRow = combo.jqxComboBox('getItems');
    if (dataRow.length > 0) {
        $.each(dataRow, function (index, item) {
            if (item.value === value)
                combo.jqxComboBox('selectIndex', index)
        });
    }
}

function fnConvertApprToString(jsonData, item, char) {
