...
function fnInitialDataSource(dataFields, url, para) {
    var source = {
        datatype: "json",
...
function fnInitialGrid(grid, source, columns, selectionmode, editable, height) {
...
function fnInitialDataSourceLocal(dataFields, localData) {
    var source = {
        datatype: "json",
        datafields: dataFields,
        localdata: localData
...
function fnInitialGridWithoutPaging(grid, source, columns, selectionmode, height) {
    var dataAdapter = new $.jqx.dataAdapter(source);
    grid.jqxGrid({
        width: "100%",
        height: height,
